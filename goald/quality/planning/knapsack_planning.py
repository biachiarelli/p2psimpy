...
class Planning:
            ...
            for dep in dependencies:
                if(dep.myType() is Refinement().GOAL):
                    ...
                    if self.isAchievableTask(dep, current, interp):
                        ...
                        # map of valid(context active) tasks 
                        mapTasks.append({'task': dep, 'qc': qc})
                        # list of values for the knapsack calc
                        value.append(1/qc)
                       
            if len(mapTasks) > 0:
                task = self.knapSack(interp, mapTasks, value)
                plan = Plan()
    ...
    def knapSack(self, interp, mapTasks, val): 
        ...
        for i in range(n + 1): 
            for w in range(capacity + 1): 
                if i == 0 or w == 0: 
                    K[i][w] =  {'val': 0, 'task': None}
                elif mapTasks[i-1]['qc'] <= w: 
                    outKnapsack = K[i-1][w]
                    thisVal = val[i-1] 
                    pastVal = K[i-1][w-mapTasks[i-1]['qc']]['val']
                    newVal = thisVal + pastVal
                    task = [mapTasks[i-1]['task']]

                    if pastVal != 0:
                        task.append(K[i-1][w-mapTasks[i-1]['qc']]['task'][0])

                    inKnapsak = {'val': newVal, 'task': task}
                    if inKnapsak['val'] > outKnapsack['val']:
                        K[i][w] = inKnapsak
                    else:
                        K[i][w] = outKnapsack
                else: 
